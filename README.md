...
각도를 0~1000 으로 바꾸는 기준값은 **실측**으로 정했습니다(원자료: [`docs/측정자료/각도분포_실측.txt`](./docs/측정자료/각도분포_실측.txt)). 실행 중 이 범위를 벗어나는 각이 나오면 자동으로 **넓어집니다**. 좁히지는 않습니다 — 자동 축소를 넣으면 손을 가만히 두었을 때 기준폭이 오그라들어 미세한 떨림이 0~1000 전체로 증폭됩니다. 다시 잡으려면 실행 중 `c` 키를 누릅니다.
