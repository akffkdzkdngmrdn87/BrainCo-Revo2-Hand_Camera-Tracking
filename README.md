...
>
> **Copyright (c) 2026 pi2jw. All rights reserved. (독점 라이선스)**
> 저작권자의 **사전 서면 허락 없이** 상업적 이용, 수정(개작·번역·2차적 저작물 작성),
> 재배포(복제물 배포·공중송신·다른 저장소나 제품에의 포함)를 할 수 없습니다.
> **포크·복사본 게시·수정본 공개도 재배포에 해당합니다.**
> 허용하는 것은 **개인적이고 비상업적인 열람·실행·학습**뿐입니다.
> 자세한 조건은 **[`LICENSE`](./LICENSE)** 와 아래 **10장(라이선스 · 상표 · 특허)** 을 보십시오.
>
> ★**제3자 구성요소**(Google 손 랜드마크 모델 · BrainCo SDK 등)에는 이 제한이 적용되지 않으며
...
## 🪟 윈도우에서 쓰시려면 — [`windows판/`](./windows판) 폴더만 받으면 됩니다

이 저장소에는 **두 가지 판**이 들어 있습니다. 서로 건드리지 않습니다.

| 판 | 어디에 | 무엇이 다른가 |
|---|---|---|
| **Ubuntu 판** | 저장소 최상위 (`src/` · `설치.sh` · `실행_*.sh`) | 원본. 아래 문서 전체가 이 판을 설명합니다 |
| **Windows 판** | **[`windows판/`](./windows판)** | 두 번 눌러 실행하는 `.bat` · 파이썬 자동 설치 · 드라이버 도우미 · 제조사 SDK 동봉 |

**쓰는 법 — `windows판/` 폴더의 파일을 내려받아 적용하면 됩니다.**

1. 이 저장소를 내려받습니다 (`Code ▾ → Download ZIP`, 또는 `git clone`)
2. 압축을 풀고 **`windows판/` 폴더만** 원하는 자리로 옮깁니다 — 그 폴더 하나로 완결됩니다
3. **`로봇손_따라하기.bat`** 을 두 번 누릅니다
   · 파이썬이 없으면 **계정 안에만** 설치합니다 (시스템을 건드리지 않습니다)
   · 필요한 꾸러미와 모델을 갖춥니다
   · 로봇손이 안 잡히면 무엇이 문제인지 가려서 알려 줍니다
4. 자세한 설명은 [`windows판/사용법.md`](./windows판/사용법.md)

> **⚠ 로봇손 드라이버는 끝까지 대신 깔아 드리지 않습니다.**
> 먼저 **윈도우 업데이트 → 제조사 공식 설치 파일** 순서로 **자동 설치를 시도**하고,
> 그래도 안 되면 **받는 곳을 열어 주고 손으로 설치하는 방법을 단계별로 안내**합니다.
> 드라이버는 관리자 권한이 필요하고 배포 주소가 바뀌기도 해서, 끝까지 자동화하는 것이
> 오히려 위험하기 때문입니다. **관리자 권한 창(UAC)은 반드시 사람이 승인합니다.**
>
> | 직접 받는 곳 | 무엇 |
> |---|---|
...
**확인된 환경**: Windows 10 · Windows 11 (데스크톱 PC · 노트북에서 정상 구동 확인)
...
| ⑥ | **거울 화면에서 왼손·오른손이 반대로 표시됨** | 같은 영상을 좌우만 뒤집어 넣으니 판정이 360프레임 전부 뒤집힘 | 거울을 걸었으면 **되돌린다**(§6.6) · `h` 키로 즉시 전환 |
...
### 6.6 손 좌우(handedness)는 거울 화면에서 뒤집힌다

MediaPipe 는 손 좌우를 **화면에 보이는 모양 그대로** 판정합니다.
공식 문서에는 「거울상 입력을 가정한다」고 되어 있으나, **실측은 그렇지 않았습니다.**

같은 영상을 좌우만 뒤집어 넣어 보았습니다.

| 입력 | 판정 | 프레임 | 확신도 |
|---|---|---|---|
| 원본 (거울 아님) | `Left` | 360 / 360 | 0.949 |
| 좌우 반전 | `Right` | 360 / 360 | 0.823 |

영상 속 손은 **손바닥이 보이는 왼손**이므로, **거울이 아닌 입력에서는 판정이 맞고
거울 입력에서는 뒤집힙니다.**

그런데 이 프로그램은 카메라 화면을 **거울 모드**(`cv2.flip`)로 보여 줍니다.
내 손과 화면이 같은 쪽으로 움직여야 쓰기 편하기 때문입니다. 그 결과 화면 속 손은
실제와 좌우가 뒤집힌 모양이 되고, MediaPipe 도 뒤집힌 대로 판정했습니다.

**그래서 거울을 걸었으면 판정을 되돌립니다.** 화면은 거울로 두어 쓰기 편하게 하면서,
손 이름은 실제와 같아집니다.

```bash
--손좌우 자동     # 기본. 거울 모드면 되돌린다
--손좌우 바꿈     # 항상 뒤집는다
--손좌우 그대로   # MediaPipe 값을 손대지 않는다
```
실행 중에는 **`h` 키**로 바꿀 수 있습니다.

> **영상 파일 경로는 거울을 걸지 않으므로 원래 맞았습니다** — 카메라에서만 뒤집혔습니다.
> **손가락 움직임(로봇 제어)에는 영향이 없습니다.** 손 이름 표시와 CSV 열에만 쓰입니다.
...
├─ LICENSE                       ⛔독점 라이선스 (상업적 이용·수정·재배포 금지)
├─ NOTICE                        저작권 표시 · 제3자 구성요소와 그 라이선스
...
└─ windows판/                    ★윈도우 판 — 이 폴더 하나로 완결됩니다
    ├─ 로봇손_따라하기.bat         두 번 눌러 실행 (웹캠 → 로봇)
    ├─ 웹캠만_보기.bat             로봇 없이 화면만
    ├─ 작은창으로_따라하기.bat      작은 창으로
    ├─ 연결진단.bat                로봇손이 안 잡힐 때 원인을 가린다
    ├─ 환경조사.bat                이 PC 의 파이썬·카메라·포트를 조사
    ├─ 사용법.md                   윈도우 사용 설명서
    ├─ _setup.py                  파이썬·꾸러미·모델 맞춤 설치
    ├─ _driver.py                 드라이버 자동 설치 시도 → 안 되면 안내
    ├─ _cams.py · _ports.py       카메라·COM 포트 찾기
    ├─ tests_*.py                 윈도우 분기 시험 (기체·로봇 없이 돈다)
    ├─ src/hand_tracking_control.py  본체 (Ubuntu 판에 윈도우 분기를 더한 판)
    ├─ models/ · media/           모델과 검증용 입력 영상
    └─ brainco-hand-sdk/          제조사 SDK (MIT) — 별도 설치가 필요 없습니다
```

## 10. 라이선스 · 상표 · 특허 (License, Trademarks & Patents)

### 10.1 본 저장소 — ⛔상업적 이용·수정·재배포를 허용하지 않습니다

```
Copyright (c) 2026 pi2jw (akffkdzkdngmrdn87). All rights reserved.
모든 권리 보유 — 독점 라이선스 (Proprietary License)
```

> ## ⛔ 저작권자의 사전 서면 허락 없이는 다음을 할 수 없습니다
>
> | 금지 | 무엇을 뜻하나 |
> |---|---|
> | **상업적 이용** | 판매·대여·유상 서비스·광고 수익, **사내 업무에의 활용**을 포함한 **일체의 영리 목적** |
> | **수정** | 개작·번역·편집, **2차적 저작물의 작성** |
> | **재배포** | 복제물의 배포·공중송신·전시, **다른 저장소·매체·제품에의 포함이나 재게시** |
>
> **허용하는 것은 개인적이고 비상업적인 열람·실행·학습뿐입니다.**
> 포크(fork)·복사본 게시·수정본 공개도 재배포에 해당합니다.
> 위 허락이 필요하면 <https://github.com/akffkdzkdngmrdn87> 로 문의하십시오.

전문은 **[`LICENSE`](./LICENSE)**, 제3자 구성요소 목록은 **[`NOTICE`](./NOTICE)** 에 있습니다.

**★제3자 구성요소에는 위 제한이 적용되지 않습니다.** 이 저장소가 함께 담아 배포하는
제3자 저작물(Google 의 손 랜드마크 모델, BrainCo 의 SDK 등)에는 **각자의 원래 라이선스**가
그대로 적용되며, 본 저장소는 그 권리를 제한하지 않습니다. 10.2 를 보십시오.

> **⚠ 종전 배포분에 관하여** — 2026-09-22 이전에 **Apache License 2.0** 으로 배포된 판
> (`v1.0.0` 및 해당 Zenodo 기록)은 **그 시점의 라이선스가 그대로 적용**됩니다.
> 이미 그 조건으로 내려받은 이용자의 권리는 소급하여 제한되지 않습니다.
> 위 조건은 **그 이후에 배포되는 판부터** 적용됩니다.

### 10.2 제3자 구성요소 — 각자의 원래 라이선스가 적용됩니다
...
둘 다 **원본을 고치지 않고 그대로** 담았으며, MIT SDK 는 라이선스 전문을 함께 둡니다.

**■ 담지 않고, 설치할 때 각 배포처에서 내려받는 것**
...
| `numpy` | BSD 3-Clause |
| `Pillow` | MIT-CMU |
| `bc-stark-sdk` | MIT License (BrainCoTech) |
...
`media/` 의 영상과 이미지는 **저작권자가 직접 촬영**한 것이며, 10.1 의 조건이 적용됩니다.
...
> 침해 여부는 본 저장소가 판단할 수 없으며, [`LICENSE`](./LICENSE) 제4조·제5조에 따라
> **어떠한 보증도 제공하지 않습니다.** 로봇을 구동하는 소프트웨어이므로
> 기기 손상·부상의 위험이 있습니다. 이용에 따르는 책임은 이용자에게 있습니다.
> **상업적 이용은 10.1 에 따라 저작권자의 사전 서면 허락이 필요하며**, 허락을 받은
> 경우에도 제3자 특허에 대해서는 별도의 법률 자문을 받으시기 바랍니다.
