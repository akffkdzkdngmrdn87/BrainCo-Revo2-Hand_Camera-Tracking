...
## 🪟 윈도우에서 쓰시려면 — [`windows/`](./windows) 폴더만 받으면 됩니다
...
| **Windows 판** | **[`windows/`](./windows)** | 두 번 눌러 실행하는 `.bat` · 파이썬 자동 설치 · 드라이버 도우미 · 제조사 SDK 동봉 |

**쓰는 법 — `windows/` 폴더의 파일을 내려받아 적용하면 됩니다.**

1. 이 저장소를 내려받습니다 (`Code ▾ → Download ZIP`, 또는 `git clone`)
2. 압축을 풀고 **`windows/` 폴더만** 원하는 자리로 옮깁니다 — 그 폴더 하나로 완결됩니다
3. **`로봇손_따라하기.bat`** 을 두 번 누릅니다
   · 파이썬이 없으면 **계정 안에만** 설치합니다 (시스템을 건드리지 않습니다)
   · 필요한 꾸러미와 모델을 갖춥니다
   · 로봇손이 안 잡히면 무엇이 문제인지 가려서 알려 줍니다
4. 자세한 설명은 [`windows/사용법.md`](./windows/사용법.md)
