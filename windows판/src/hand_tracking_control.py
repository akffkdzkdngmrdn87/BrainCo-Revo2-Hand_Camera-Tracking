...
# 운영체제 차이 — 한 곳에서만 흡수한다
#
# ★리눅스와 윈도우는 카메라를 여는 방법이 다르다 (2026-09-10 윈도우 판을 만들며 확인)
#     리눅스 : V4L2 백엔드를 **명시해야** 한다. 생략하면 OpenCV 가 실패한 뒤
#              FFMPEG 로 되돌아가 메타데이터 전용 장치까지 「사용 가능」으로
#              잘못 보고한다.
#     윈도우 : V4L2 가 아예 없다. `cv2.CAP_V4L2` 를 그대로 쓰면 카메라가
#              열리지 않는다. DirectShow(CAP_DSHOW)를 쓴다. MSMF 는 장치에
#              따라 첫 프레임이 늦거나 실패하는 일이 있어 기본으로 두지 않는다.
#   장치 목록도 다르다. 리눅스는 /dev/video* 를 훑지만 윈도우에는 그런 경로가
#   없어 인덱스를 차례로 열어 보아야 한다.
# ─────────────────────────────────────────────────────────────────────────────

IS_WINDOWS = (os.name == "nt")


WIN_NAME = "BrainCo Revo2 - 손추적 제어  (q=종료  f=전체화면)"


def set_fullscreen(win, on):
    """창을 전체 화면으로 바꾸거나 되돌린다.

    ★전체 화면에서도 화면이 흐리지 않게 카메라를 720p 로 받는다(CAM_SIZES).
      640x480 을 1920 폭으로 늘리면 세 배로 확대되어 눈에 띄게 뭉갠다.
    """
    import cv2
    try:
...
def camera_backends():
    """카메라를 열 때 시도할 백엔드를 순서대로 돌려준다. [(상수, 이름), ...]

    ★하나만 시도하면 안 된다 (2026-09-10 윈도우에서 실제로 막혔다)
      윈도우에서 어떤 장치는 DirectShow 로, 어떤 장치는 Media Foundation
      으로만 열린다. 노트북 내장 카메라·가상 카메라·USB 웹캠이 서로 다르다.
      하나만 쓰면 「카메라가 없다」는 잘못된 결론에 이른다.
      마지막의 CAP_ANY 는 OpenCV 가 알아서 고르게 하는 것이다.
    """
    import cv2
    if IS_WINDOWS:
...
def camera_backend():
    """(옛 이름 유지) 첫 번째 백엔드 상수."""
    return camera_backends()[0][0]


def camera_backend_name():
    return camera_backends()[0][1]


# 카메라에서 받을 크기. 전체 화면으로 크게 띄우므로 640x480 은 흐릿하다.
#   대부분의 웹캠이 720p 를 지원하고, MediaPipe 는 내부에서 축소해 처리하므로
#   손 인식 속도에는 큰 영향이 없다. 안 되는 카메라는 아래에서 자동으로 낮춘다.
CAM_SIZES = [(1280, 720), (640, 480)]


def open_camera(idx, sizes=None):
    """카메라를 연다. 백엔드를 차례로 시도한다.

    「열렸다」의 기준은 isOpened() 가 아니라 **한 장이 실제로 읽히는가** 다.
    윈도우에서는 열리기만 하고 영상이 안 나오는 장치가 흔하다.

    ★★순서가 대단히 중요하다 (2026-09-10 실제 사고)
      **① 아무것도 설정하지 않고 먼저 열어서 읽히는지 확인**한다.
      **② 그다음에** 화면을 크게 띄우려고 해상도를 올려 본다.
      **③ 올리다 실패하면 카메라를 다시 열어 ①의 상태로 되돌린다.**

      해상도를 먼저 요구하면, 그 크기를 못 내는 웹캠에서 **멀쩡한 카메라를
      「없다」고 잘못 판단**한다. 실제로 640x480 웹캠에서 720p 를 먼저
      요구했다가 잘 되던 프로그램이 실행조차 되지 않았다.
      해상도는 **보기 좋으라고 올리는 덤**이지 동작 조건이 아니다.
      덤 때문에 본래 기능을 잃어서는 안 된다.

      ③에서 그냥 원래 크기를 다시 set 하지 않고 **닫았다 다시 여는** 이유는,
      DirectShow 에서 해상도 변경에 실패한 손잡이는 그 뒤로 아무 크기에서도
      영상을 내주지 않는 경우가 있기 때문이다.
    """
    import cv2
    시도들 = []
    for be, name in camera_backends():
        ...
        if not cap.isOpened():
            cap.release()
            시도들.append(f"{name}: 열리지 않음")
            continue

        # ① 있는 그대로 읽어 본다 — 이것이 오래 검증된 경로다
        ok, fr = cap.read()
        if not (ok and fr is not None):
            cap.release()
            시도들.append(f"{name}: 열렸으나 영상이 나오지 않음")
            continue
        기본크기 = (int(fr.shape[1]), int(fr.shape[0]))

        # ② 더 큰 크기를 원하면 올려 본다 (실패해도 잃을 것이 없어야 한다)
        for w, h in (sizes or CAM_SIZES):
            if (w, h) == 기본크기 or (w * h) <= (기본크기[0] * 기본크기[1]):
                break                     # 이미 그만큼 크다 — 건드리지 않는다
            cap.set(cv2.CAP_PROP_FRAME_WIDTH, w)
            cap.set(cv2.CAP_PROP_FRAME_HEIGHT, h)
            ok2, fr2 = cap.read()
            if ok2 and fr2 is not None:
                실제 = (int(fr2.shape[1]), int(fr2.shape[0]))
                return cap, f"{name} {실제[0]}x{실제[1]}", 시도들

            # ③ 실패 — 손잡이가 오염됐을 수 있으니 닫고 다시 연다
            시도들.append(f"{name} {w}x{h}: 안 되어 원래 크기로 되돌림")
            cap.release()
            cap = cv2.VideoCapture(idx, be)
            if not cap.isOpened():
                cap.release()
                ...
                break
            ok3, fr3 = cap.read()
            if not (ok3 and fr3 is not None):
                cap.release()
                cap = None
                break
            break                          # 되돌렸으면 더 올려 보지 않는다

        if cap is not None:
            return cap, f"{name} {기본크기[0]}x{기본크기[1]}", 시도들
        시도들.append(f"{name}: 되돌리기에도 실패")
    return None, None, 시도들
...
FONT_CANDIDATES = [
    # ★윈도우 글꼴을 먼저 본다 (2026-09-10)
    #   리눅스 경로만 두면 윈도우에서 한글이 모두 네모로 깨진다.
    #   맑은 고딕은 윈도우 비스타 이후 기본 탑재라 별도 설치가 필요 없다.
    #   (리눅스에서는 이 경로들이 없으므로 그냥 건너뛴다 — 서로 방해하지 않는다)
    r"C:\Windows\Fonts\malgunbd.ttf",      # 맑은 고딕 볼드
    r"C:\Windows\Fonts\malgun.ttf",        # 맑은 고딕
    r"C:\Windows\Fonts\NanumGothicBold.ttf",
    r"C:\Windows\Fonts\gulim.ttc",         # 굴림 (구형 윈도우)
...
class SdkBackend(BackendBase):
    ...
    def __init__(self, sdk_dir=None, port=None):
        self.sdk_dir = sdk_dir
        # 포트 이름. None 이면 SDK 가 스스로 찾는다.
        #   리눅스 예: /dev/ttyUSB0     윈도우 예: COM3
        self.port = port
    ...
    #   윈도우에서는 zip 을 푼 폴더 옆이나 사용자 폴더에 두는 일이 많다.
    #   os.path.expanduser 가 윈도우에서도 %USERPROFILE% 을 풀어 주므로
    #   같은 표기를 쓰되, 프로그램 폴더 옆도 함께 찾는다.
    SDK_CANDIDATES = [
        "~/brainco-hand-sdk",
        ...
        os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                     "brainco-hand-sdk-main"),
        "~/Downloads/brainco-hand-sdk",
        "~/Desktop/brainco-hand-sdk",
    ]
    ...
    def _worker(self):
        ...
        async def run():
            # port 가 None 이면 SDK 가 첫 번째로 찾은 포트를 쓴다.
            # 포트가 여럿인 컴퓨터(윈도우에서 흔하다)에서는 --port 로 지정한다.
            client, slave_id = await open_modbus_revo2(self.port)
            self.slave_id = slave_id
            self.client = client
    ...
    def connect(self):
        ...
        if self.error:
            ...
            if IS_WINDOWS:
                raise SystemExit(
                    f"[오류] 로봇손 연결 실패: {self.error}\n"
                    f"       ① 24V DC 어댑터 연결 확인 (가장 흔한 원인)\n"
                    f"          USB 는 통신 전용이라 전원 없이는 응답하지 않습니다\n"
                    f"       ② 장치 관리자 → 포트(COM & LPT) 에서 COM 번호를 확인하고\n"
                    f"          --port COM3 처럼 지정하십시오\n"
                    f"          번호가 안 보이면 USB 직렬 드라이버(CP210x/CH340)를 설치하십시오\n"
                    f"       ③ 다른 프로그램이 그 COM 포트를 잡고 있는지 확인하십시오")
...
def make_backend(kind: str, slave_id: int, sdk_dir, verbose: bool,
                 port=None) -> BackendBase:
    if kind == "ros2":
        return Ros2Backend(slave_id=slave_id)
    if kind == "sdk":
        return SdkBackend(sdk_dir=sdk_dir, port=port)
...
class HandTracker:
    ...
    def __init__(self, model_path, num_hands=1, det_conf=0.5, pres_conf=0.5,
        ...
        # ★모델을 경로가 아니라 **바이트로** 넘긴다 (2026-09-10 윈도우에서 겪음)
        #   MediaPipe 는 C++ 라이브러리라 윈도우에서 **비ASCII 경로를 열지 못한다.**
        #   폴더 이름에 한글이 하나만 있어도 이렇게 죽는다.
        #       RuntimeError: Unable to open file at
        #         C:\Users\...\손추적_win\models\hand_landmarker.task, errno=-1
        #   파일 읽기는 파이썬이 하고 내용만 넘기면 경로가 아예 관여하지 않는다.
        #   (리눅스는 UTF-8 이 기본이라 이 문제가 드러나지 않는다)
        try:
            with open(model_path, "rb") as fh:
                model_buf = fh.read()
        except OSError as e:
            raise SystemExit(
                f"[오류] 손 인식 모델을 읽지 못했습니다: {model_path}\n"
                f"       {type(e).__name__}: {e}\n"
                f"       압축을 풀 때 models 폴더가 함께 풀렸는지 확인하십시오.")
        if len(model_buf) < 1_000_000:
            raise SystemExit(
                f"[오류] 모델 파일이 온전하지 않습니다 "
                f"({len(model_buf):,} 바이트, 7MB 안팎이어야 합니다).\n"
                f"       {model_path}\n"
                f"       압축을 다시 풀어 주십시오.")
        opts = vision.HandLandmarkerOptions(
            base_options=mpp.BaseOptions(model_asset_buffer=model_buf),
            running_mode=mode,
            num_hands=num_hands,
...
def open_source(source: str, cam_sizes=None):
    """
    --source 문자열을 해석해 (VideoCapture, 종류, 총프레임수, fps) 를 돌려준다.
    종류: "camera" | "video" | "image"
    cam_sizes 를 주면 카메라를 그 크기로 열어 본다(앞에서부터 시도).
    """
    import cv2
    if source.isdigit():
        idx = int(source)
        ...
        if os.name == "posix" and not os.path.exists(dev):
            raise SystemExit(
                ...
                f"         --source 어떤영상.mp4")
        # 백엔드를 차례로 시도한다(윈도우는 장치마다 되는 백엔드가 다르다)
        cap, 쓴백엔드, 시도들 = open_camera(idx, cam_sizes)
        if cap is None:
            보고 = "".join(f"         · {t}\n" for t in 시도들)
            if IS_WINDOWS:
                raise SystemExit(
                    f"[오류] 카메라 {idx} 번을 열지 못했습니다.\n"
                    f"       시도한 방법:\n{보고}"
                    f"       ① 다른 프로그램(줌·팀즈·카메라 앱)을 모두 닫으십시오\n"
                    f"       ② 설정 → 개인 정보 → 카메라 →\n"
                    f"          「앱이 카메라에 액세스하도록 허용」과\n"
                    f"          「데스크톱 앱이 카메라에 액세스하도록 허용」을 켜십시오\n"
                    f"       ③ 다른 번호를 써 보십시오: --source 1  또는  --source 2\n"
                    f"       ④ 쓸 수 있는 장치 목록: --list-cameras")
            raise SystemExit(
                f"[오류] 카메라 {idx} 번을 열지 못했습니다.\n"
                f"       시도한 방법:\n{보고}"
                f"       ① 다른 프로그램이 쓰고 있는지 확인: fuser -v {dev}\n"
                f"       ② 쓸 수 있는 장치 목록 확인: --list-cameras")
        print(f"  카메라 {idx} 번을 {쓴백엔드} 로 열었습니다.")
    ...
    if not cap.isOpened() and not path.isascii():
        # ★OpenCV 도 윈도우에서 비ASCII 경로를 못 여는 일이 있다.
        #   내용은 멀쩡하므로 ASCII 이름의 임시 파일로 복사해 다시 연다.
        import shutil
        import tempfile
        try:
            임시 = os.path.join(tempfile.gettempdir(),
                              "hand_input" + os.path.splitext(path)[1])
            shutil.copy2(path, 임시)
            cap = cv2.VideoCapture(임시)
            if cap.isOpened():
                print(f"  (경로에 한글이 있어 임시로 복사해 엽니다: {임시})")
        except Exception:
            pass
    if not cap.isOpened():
        raise SystemExit(
            f"[오류] 영상 파일을 열 수 없습니다: {path}\n"
            f"       파일이 온전한지, 경로에 한글이 있는지 확인하십시오.")
...
#  손 좌우(handedness) 보정
# ─────────────────────────────────────────────────────────────────────────
def 손좌우뒤집기(handed: str) -> str:
    """`Left` ↔ `Right` 를 맞바꾼다. 빈 값이나 모르는 값은 그대로 둔다."""
    return {"Left": "Right", "Right": "Left"}.get(handed, handed)


def 손좌우보정(handed: str, 뒤집을까: bool) -> str:
    """MediaPipe 가 낸 손 좌우를 실제 손에 맞게 고친다.

    ★왜 필요한가 (2026-09-22 j2w 보고 → 실측으로 원인 확정)
      MediaPipe 는 **화면에 보이는 모양 그대로** 좌우를 판정한다.
      같은 영상을 좌우만 뒤집어 넣어 보니 판정도 정확히 뒤집혔다.
          원본   360프레임 전부 `Left`  (확신도 0.949)
          좌우반전 360프레임 전부 `Right` (확신도 0.823)
      그런데 이 프로그램은 카메라 화면을 **거울 모드**(`cv2.flip`)로 보여 준다.
      내 손과 화면이 같은 쪽으로 움직여야 쓰기 편하기 때문이다.
      그 결과 **화면 속 손은 실제와 좌우가 뒤집힌 모양**이 되고,
      MediaPipe 도 뒤집힌 대로 판정해 **왼손을 오른손이라고 표시**했다.

      그래서 **거울을 걸었으면 판정을 되돌린다.** 이렇게 하면 화면은 거울로
      두면서(쓰기 편하게) 손 이름은 실제와 같아진다.

    ★영상 파일은 거울을 걸지 않으므로 보정하지 않는다 — 원래 맞았다.
    """
    if not handed or not 뒤집을까:
        return handed
    return 손좌우뒤집기(handed)
...
def run_self_test() -> int:
    ...
    # ★2026-09-22 — 거울 모드에서 왼손·오른손이 반대로 나오던 결함을 막는다.
    #   같은 영상을 좌우만 뒤집어 넣으면 MediaPipe 판정도 그대로 뒤집힌다(실측).
    #   그래서 거울을 걸었으면 되돌려야 한다.
    print("\n[9] 손 좌우 보정 검증")
    경우 = [
        ("거울을 걸었으면 되돌린다",        "Left",  True,  "Right"),
        ("거울을 걸었으면 되돌린다",        "Right", True,  "Left"),
        ("거울이 아니면 그대로 둔다",       "Left",  False, "Left"),
        ("거울이 아니면 그대로 둔다",       "Right", False, "Right"),
        ("손을 못 찾았으면 건드리지 않는다", "",      True,  ""),
    ]
    for 설명, 들어온값, 뒤집을까, 기대 in 경우:
        얻음 = 손좌우보정(들어온값, 뒤집을까)
        ok = (얻음 == 기대)
        print(f"    {'통과' if ok else '실패'}  {설명}: "
              f"{들어온값 or '(없음)'} → {얻음 or '(없음)'} (기대 {기대 or '(없음)'})")
        if not ok:
            fails.append("손 좌우 보정")
    ok = 손좌우뒤집기("Left") == "Right" and 손좌우뒤집기("Right") == "Left"
    print(f"    {'통과' if ok else '실패'}  Left ↔ Right 맞바꿈")
    if not ok:
        fails.append("손 좌우 맞바꿈")
...
def build_parser():
    ...
    p.add_argument("--port", default=None,
                   help="로봇손이 붙은 직렬 포트. 비우면 SDK 가 스스로 찾는다. "
                        "리눅스 예 /dev/ttyUSB0 · 윈도우 예 COM3. "
                        "포트가 여럿이면 지정해야 엉뚱한 장치를 잡지 않는다")
    ...
    p.add_argument("--손좌우", choices=["자동", "그대로", "바꿈"], default="자동",
                   help="화면에 표시할 손 이름(Left/Right)을 정하는 방법. "
                        "자동=거울 모드면 되돌린다(권장) · 그대로=MediaPipe 값 그대로 · "
                        "바꿈=항상 뒤집는다. 실행 중 h 키로도 바꿀 수 있다")
    ...
    p.add_argument("--windowed", action="store_true",
                   help="전체 화면이 아니라 보통 창으로 띄운다. "
                        "돌리는 중에 f 키로 언제든 바꿀 수 있다")
    p.add_argument("--cam-size", default=None, metavar="가로x세로",
                   help="카메라에서 받을 크기. 기본은 1280x720 을 먼저 시도하고 "
                        "안 되면 640x480 으로 내린다. 예) --cam-size 640x480")
    ...
    p.add_argument("--list-ports", action="store_true",
                   help="로봇손이 붙을 수 있는 직렬 포트를 보여 주고 끝낸다. "
                        "윈도우에서 COM 번호를 모를 때 쓴다")
    return p


def list_ports(sdk_dir=None):
    """로봇손이 붙을 수 있는 직렬 포트를 찾아 보여 준다.

    ★윈도우에서 이것이 없으면 사람이 막힌다 (2026-09-10)
      리눅스는 /dev/ttyUSB0 하나뿐인 일이 많지만, 윈도우는 COM 번호가
      컴퓨터마다 다르고 블루투스·가상 포트까지 섞여 있다. 번호를 모르면
      --port 에 무엇을 넣어야 할지 알 수 없다. 그래서 제조사 SDK 가 주는
      목록을 그대로 보여 준다.
    """
    print("로봇손이 붙을 수 있는 직렬 포트를 찾습니다.")
    # SDK 를 찾아 sys.path 에 넣는다 (SdkBackend 와 같은 방법)
    cands = []
    if sdk_dir:
        cands.append(sdk_dir)
    if os.environ.get("STARK_SDK_DIR"):
        cands.append(os.environ["STARK_SDK_DIR"])
    cands += SdkBackend.SDK_CANDIDATES
    revo2 = None
    for d in cands:
        cand = os.path.join(os.path.expanduser(d), "python", "revo2")
        if os.path.isdir(cand):
            revo2 = cand
            break
    if revo2 is None:
        print("  [오류] 제조사 SDK 예제 폴더를 찾지 못했습니다.")
        print("         로봇손 없이 웹캠만 쓰신다면 이 항목은 넘어가셔도 됩니다.")
        return 1
    if revo2 not in sys.path:
        sys.path.insert(0, revo2)
    try:
        ...
        print(f"  [오류] 로봇 통신 꾸러미를 불러오지 못했습니다: {e}")
        print("         pip install bc-stark-sdk 가 되어 있는지 확인하십시오.")
        return 1

    try:
        raw = libstark.list_available_ports()
        text = raw.decode("utf-8") if isinstance(raw, (bytes, bytearray)) else str(raw)
        ports = _json.loads(text)
    except Exception as e:
        print(f"  [오류] 포트 목록을 읽지 못했습니다: {e}")
        return 1

    if not ports:
        print("  없음. 로봇손이 연결되어 있지 않습니다.")
        print("  → ① USB 케이블을 꽂으십시오")
        print("     ② ★24V DC 어댑터를 꽂으십시오 (USB 는 통신 전용입니다)")
        if IS_WINDOWS:
            print("     ③ 장치 관리자 → 포트(COM & LPT) 에 항목이 보이는지 확인하십시오")
            print("        안 보이면 USB 직렬 드라이버(CP210x 또는 CH340)를 설치하십시오")
        else:
            print("     ③ ls /dev/ttyUSB* 로 확인하고, 권한은 dialout 그룹입니다")
        return 1

    print(f"  {len(ports)}개를 찾았습니다:")
    for it in ports:
        if isinstance(it, dict):
            이름 = it.get("port_name") or it.get("name") or it.get("port") or str(it)
            덧 = " ".join(f"{k}={v}" for k, v in it.items()
                         if k not in ("port_name", "name", "port"))
            print(f"    {이름}    {덧}")
        else:
            print(f"    {it}")
    print()
    print("  이 중 로봇손이 붙은 포트를 --port 로 지정하십시오.")
    print("    예)  --port " + ("COM3" if IS_WINDOWS else "/dev/ttyUSB0"))
    print("  하나뿐이면 지정하지 않아도 SDK 가 스스로 찾습니다.")
    return 0


def list_cameras():
    """쓸 수 있는 카메라를 찾아 보여 준다.

    ★리눅스와 윈도우는 찾는 방법이 다르다.
        리눅스 : /dev/video* 가 장치 목록이다. 이름까지 그대로 보여 줄 수 있다.
        윈도우 : 그런 경로가 없다. 인덱스 0 부터 차례로 열어 보는 수밖에 없다.
                 열리지 않는 번호가 나와도 그 뒤에 장치가 있을 수 있으므로
                 몇 개는 더 시도한다.
    """
    ...
    if IS_WINDOWS:
        MAX_IDX = 8
        found_any = False
        print("  (DirectShow → MediaFoundation → 자동 순서로 시도합니다)")
        for idx in range(MAX_IDX):
            cap, 쓴백엔드, 시도들 = open_camera(idx)   # 목록 조회는 기본 크기 후보로 시도한다
            if cap is not None:
                found_any = True
                ok, fr = cap.read()
                if ok and fr is not None:
                    print(f"  카메라 {idx}번  사용 가능  "
                          f"해상도 {fr.shape[1]}x{fr.shape[0]}  "
                          f"({쓴백엔드})  → --source {idx}")
                    usable += 1
                else:
                    print(f"  카메라 {idx}번  열렸지만 영상이 나오지 않습니다")
                cap.release()
            elif 시도들 and any("열렸으나" in t for t in 시도들):
                found_any = True
                print(f"  카메라 {idx}번  열리지만 영상이 나오지 않습니다: "
                      + " / ".join(시도들))
        if not found_any:
            print("  없음. 이 컴퓨터에는 웹캠이 연결되어 있지 않습니다.")
            print("  → USB 웹캠을 꽂거나, --source 영상파일.mp4 로 시험하십시오.")
            print("  → 노트북 내장 카메라가 있는데도 안 보이면 설정 → 개인 정보 →")
            print("     카메라 에서 데스크톱 앱 접근을 켜십시오.")
            return 1
        if usable == 0:
...
def main(argv=None) -> int:
    ...
    if args.list_ports:
        return list_ports(args.sdk_dir)
    ...
    if args.cam_size:
        try:
            w, h = args.cam_size.lower().split('x')
            cam_sizes = [(int(w), int(h))]
        except Exception:
            print(f'[알림] --cam-size 를 읽지 못했습니다: {args.cam_size}. 기본값을 씁니다.')
    cap, kind, total, src_fps = open_source(args.source, cam_sizes=cam_sizes)
    is_image = (kind == "image")
    ...
    if not args.headless and kind != "image":
        print("\n  조작 : q 종료 | f 전체화면↔창 | c 가동범위 초기화 | "
              "space 일시정지 | o 손 펴기")
    # 리스트로 두는 이유: 아래 루프 안에서 값을 바꿔야 하는데
    # 파이썬은 안쪽 범위에서 바깥 변수에 그냥 대입할 수 없다.
    fullscreen = [not args.windowed]
    # 손 좌우 표시 방법 — 실행 중 h 키로 바꿀 수 있어 리스트에 담는다
    손좌우모드 = [args.손좌우]
    _win_ready = [False]
    print("=" * 74 + "\n")
    ...
    try:
        while True:
            ...
            거울켬 = (kind == "camera" and not args.no_mirror)
            if 거울켬:
                frame = cv2.flip(frame, 1)      # 거울 모드: 내 손과 화면이 같은 쪽
            # ★거울을 걸면 화면 속 손이 실제와 좌우가 뒤집힌 모양이 되어
            #   MediaPipe 도 뒤집힌 대로 판정한다. 그래서 되돌린다(손좌우보정 참조).
            if 손좌우모드[0] == "자동":
                손좌우뒤집을까 = 거울켬
            elif 손좌우모드[0] == "바꿈":
                손좌우뒤집을까 = True
            ...
            if result.hand_landmarks:
                ...
                if result.handedness:
                    handed = 손좌우보정(result.handedness[0][0].category_name,
                                     손좌우뒤집을까)
            ...
            if not args.headless or args.record:
                ...
                items = []
                if result.hand_landmarks:
                    표시손 = {"Left": "왼손", "Right": "오른손"}.get(handed, handed)
                    items.append((g["pad"], g["pad"],
                                  f"손 검출  {표시손} {handed} {score:.2f}",
                                  (0, 255, 120), True))
                else:
            ...
            if not args.headless:
                # ★창을 한 번만 만든다. 매 프레임 만들면 전체 화면이 풀린다.
                if not _win_ready[0]:
                    # ★창 꾸미기가 실패해도 **영상은 반드시 나와야 한다.**
                    #   namedWindow 나 전체 화면 설정이 안 되는 환경이 있는데,
                    #   그것 때문에 프로그램이 죽으면 본래 기능을 잃는다.
                    #   실패하면 그냥 기본 창으로 계속 간다.
                    try:
                        cv2.namedWindow(WIN_NAME, cv2.WINDOW_NORMAL)
                        # 비율을 지켜 늘린다. 없으면 화면에 맞춰 찌그러진다.
                        try:
                            cv2.setWindowProperty(WIN_NAME, cv2.WND_PROP_ASPECT_RATIO,
                                                  cv2.WINDOW_KEEPRATIO)
                        except Exception:
                            pass
                        set_fullscreen(WIN_NAME, fullscreen[0])
                    except Exception as e:                     # noqa: BLE001
                        print(f"[알림] 창 설정을 적용하지 못했습니다({e}). "
                              f"기본 창으로 계속합니다.")
                        fullscreen[0] = False
                    _win_ready[0] = True
                ...
                if key == ord("q") or key == 27:
                    ...
                    set_fullscreen(WIN_NAME, fullscreen[0])
                    print(f"[알림] {'전체 화면' if fullscreen[0] else '창 모드'}로 바꿨습니다.")
                elif key == ord("c"):
                    cal.reset(); sm.reset()
                    ...
                    # ★화면에 나오는 손 이름이 실제와 반대면 이 키로 바로 바꾼다.
                    차례 = ["자동", "바꿈", "그대로"]
                    손좌우모드[0] = 차례[(차례.index(손좌우모드[0]) + 1) % len(차례)]
                    print(f"[알림] 손 좌우 표시: {손좌우모드[0]}"
                          + ("  (거울 모드면 되돌립니다)" if 손좌우모드[0] == "자동" else "")
                          + ("  (항상 뒤집습니다)" if 손좌우모드[0] == "바꿈" else "")
                          + ("  (MediaPipe 값 그대로)" if 손좌우모드[0] == "그대로" else ""))
